...
- ✅ ~~브라우저로 실제 테스트카드 결제 완료 + 정산·크레딧 지급 확인 + 환불 API 호출 + 회수
  확인~~ — 완료(커밋 4df713c): 헤드리스 브라우저로 실결제 E2E 완주, 그 과정에서 환불 주문 조회
  버그(체크아웃 GET에 order_id 없음) 검출·수정. (이 체크박스는 2026-07-13 정정 — 커밋은 어제
  이미 완료했는데 여기 갱신이 누락되어 있었음.)
- ⏳ Polar 실계정 심사 제출(소셜 프로필 + 플로우 데모) — 2026-07-13 진행 상황: 프로덕션 조직
  "woong's company" 생성, 프로덕션 토큰 수령, 프로덕션 상품 6개 생성·검증 완료
  (`web/.env.production.local` 참고). 남은 심사 트리거는 정산 계좌 연결(Stripe Connect) —
  `NEEDS_FROM_OWNER.md` 🔴 항목 참고.
