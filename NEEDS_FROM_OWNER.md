...
| 🔴 | **Polar 가입 + 샌드박스 조직 access token + 웹훅 시크릿** | 결제 (신규 주 결제수단) | Phase 5(개정) 검증 | polar.sh 가입 → 샌드박스 조직 생성 → organization access token(`polar_...`) + 웹훅 엔드포인트 등록 시 발급되는 Standard Webhooks 서명 시크릿(`whsec_...`) 전달. **Stripe와 결정적 차이: Polar 샌드박스는 심사·승인 절차가 전혀 없이 가입 즉시 사용 가능** — 지금 바로 발급받아 실 체크아웃·웹훅 서명검증·환불 검증 시작 가능. `PolarPaymentProvider` 코드는 토큰 없이 작성·타입체크·테스트·빌드 전부 통과된 상태(전부 mock/offline). 토큰 수령 후 `pnpm exec tsx tools/polar-setup.ts`(web/ 에서 실행)로 6개 상품 생성→출력된 `POLAR_PRODUCT_ID_*` 라인을 web/.env에 붙여넣기 |
| ⏳ | Polar 실계정 심사 제출 (소셜 프로필 + 플로우 데모) | 실결제 활성화 | 런치 전 | Polar는 프로덕션 결제 활성화 전 판매자 심사가 있음 — 소셜 미디어 프로필, 결제 플로우 데모 등 제출 필요. 샌드박스 검증 완료 후 진행 |
| ⏳ | Polar 프로덕션 access token + 웹훅 시크릿 | 실결제 | 런치 | 심사/활성화 후 프로덕션 토큰. `POLAR_SERVER=production`으로 전환 |
